...
class AvaliadorCustoUnif(Avaliador): # herda de Avaliador
 
    '''
    AvaliadorCustoUnif implementa a Procura de Custo Uniforme: f(n) = g(n).
    Ordena a fronteira pelo custo acumulado real desde o início até ao nó.
    Não usa estimativas do custo futuro, encontra sempre o caminho mais barato.
    '''
    ...
    def prioridade(self, no): # f(n) = g(n)
        return no.custo